...
parser.add_argument(
    ...
    type=pathlib.Path,
    ######## here is path; path at l. 171
    default=pathlib.Path.cwd() / "odd_output_sEBSPs_lCTB_eM3_mH_fN_E",
)
...
######## here is rnd; path at l. 51
rnd = acts.examples.RandomNumbers(seed=4220)
...
if args.edm4hep:
    ...
    if not args.ttbar:
        ...
        addPythia8(
            ...
            # cmsEnergy=5.0*u.TeV,
            vtxGen=acts.examples.GaussianVertexGenerator(
                mean=acts.Vector4(0.0*u.mm, 0.0*u.mm, 0.0*u.mm, 0* u.ns),
                stddev=acts.Vector4(
                    0.5 * u.mm, 0.5 * u.mm, 50.0 * u.mm, 0.0 * u.ns
                ),
            ),
            rnd=rnd,
...
if args.reco:
    ...
    addSingleSeedVertexFinding(
        ...
        outputVertices="fittedSeedVerticesHough0",
        # default, no neighbors, 4 entries
        ecc=0,
    )
    ...
    addSingleSeedVertexFinding(
        s,
        outputDirRoot=outputDir,
        outputVertices="fittedSeedVerticesHough1",
    ...
    addSingleSeedVertexFinding(
        s,
        outputDirRoot=outputDir,
        outputVertices="fittedSeedVerticesHough2",
    ...
    addSingleSeedVertexFinding(
        s,
        outputDirRoot=outputDir,
        outputVertices="fittedSeedVerticesHough3",
        # no neighbors SP>=1000, 1 neighbor 200<SP<1000, 3 neighbors SP<200
        # this one is the best out of 0-4
        ecc=3,
    )
    ...
    addSingleSeedVertexFinding(
        s,
        outputDirRoot=outputDir,
        outputVertices="fittedSeedVerticesHough4",
