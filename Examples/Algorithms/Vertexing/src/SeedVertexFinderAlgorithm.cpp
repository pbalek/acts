...
ActsExamples::ProcessCode ActsExamples::SeedVertexFinderAlgorithm::execute(
  ...
  Acts::SeedVertexFinder<ActsExamples::SimSpacePoint> SeedVertexFinder1(
      zscanSeedVtxCfg1);
  ...
  ACTS_INFO("Found a vertex in the event in "
            << (t12 - t11).count() / 1e6
            << " ms, using minimalization with respect to "
            << zscanSeedVtxCfg1.minimalizeWRT);
  ACTS_INFO("Found vertex at x = " << result1[0] << "mm, y = " << result1[1]
                                   << "mm, z = " << result1[2] << "mm");
  ...
  Acts::SeedVertexFinder<ActsExamples::SimSpacePoint> SeedVertexFinder2(
      zscanSeedVtxCfg2);
  ...
  ACTS_INFO("Found a vertex in the event in "
            << (t22 - t21).count() / 1e6
            << " ms, using minimalization with respect to "
            << zscanSeedVtxCfg2.minimalizeWRT);
  ACTS_INFO("Found vertex at x = " << result2[0] << "mm, y = " << result2[1]
                                   << "mm, z = " << result2[2] << "mm");
