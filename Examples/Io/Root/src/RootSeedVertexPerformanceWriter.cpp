...
ActsExamples::RootSeedVertexPerformanceWriter::RootSeedVertexPerformanceWriter(
  ...
  if (m_outputTree == nullptr) {
    ...
    m_outputTree->Branch("allRecoZ", &m_allRecoZ);
...
ActsExamples::ProcessCode ActsExamples::RootSeedVertexPerformanceWriter::writeT(
  ...
  for (const auto& vtx : vertices) {
    ...
    std::vector<float> truePos{0.,0.,0.};

    // m_diffx.push_back(vtx.at(0) - truePos[0]);
    // m_diffy.push_back(vtx.at(1) - truePos[1]);
    // m_diffz.push_back(vtx.at(2) - truePos[2]);

    // m_truthX.push_back(truePos[0]);
    // m_truthY.push_back(truePos[1]);
    // m_truthZ.push_back(truePos[2]);

    if(counter<(int)vertices.size()-1)
    {
      m_recoX.push_back(vtx.at(0));
      m_recoY.push_back(vtx.at(1));
      m_recoZ.push_back(vtx.at(2));
    }
    else
    {
      for(const auto& zpos : vtx)
      {
        m_allRecoZ.push_back(zpos);
      }
    }
    ++counter;
