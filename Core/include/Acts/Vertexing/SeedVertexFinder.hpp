...
class SeedVertexFinder {
    ...
    private:
        ...
        struct Triplet{
            Triplet(const spacepoint_t* aa, const spacepoint_t* bb, const spacepoint_t* cc) : a(aa), b(bb), c(cc), ray(Acts::Vector3::Zero(),Acts::Vector3::Zero()) {}
        ...
        std::vector<std::vector<std::vector<const spacepoint_t*>>> sortSpacepoints(const std::vector<spacepoint_t>& spacepoints) const;
        ...
        std::vector<Triplet> findTriplets(const std::vector<std::vector<std::vector<const spacepoint_t*>>>& sorted_spacepoints) const;
        ...
        ///         If "minimalizeWRT"=="rays", then the fitted ray is also saved to the triplet for later
        ...
        std::pair<Acts::Vector3,Acts::ActsScalar> makePlaneFromTriplet(const Triplet triplet) const;
