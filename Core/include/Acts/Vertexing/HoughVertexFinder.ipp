...
Acts::Result<Acts::Vertex> Acts::HoughVertexFinder<spacepoint_t>::find(
  ...
  for (std::uint32_t iter = 0; iter < m_cfg.rangeIterZ.size(); ++iter) {
    auto vtxNew =
        findHoughVertex(spacepoints, vtx, m_cfg.rangeIterZ.at(iter),
                        m_cfg.nBinsZIterZ.at(iter), minCotTheta, maxCotTheta,
                        m_cfg.nBinsCotThetaIterZ.at(iter) / binsNumDecrease);
