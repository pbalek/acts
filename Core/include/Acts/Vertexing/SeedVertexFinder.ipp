...
Acts::SeedVertexFinder<spacepoint_t>::SeedVertexFinder(
  ...
  // std::transform(cfg.minimalizeWRT.begin(), cfg.minimalizeWRT.end(),
  // cfg.minimalizeWRT.begin(), [](unsigned char c){ return std::tolower(c); });
...
Acts::Vector3 Acts::SeedVertexFinder<spacepoint_t>::findVertex(
  ...
  std::vector<std::vector<std::vector<const spacepoint_t*>>>
...
Acts::SeedVertexFinder<spacepoint_t>::sortSpacepoints(
  ...
  std::vector<std::vector<const spacepoint_t*>> near_spacepoints(
      m_cfg.numPhiSlices, helper),
      middle_spacepoints(m_cfg.numPhiSlices, helper),
      far_spacepoints(m_cfg.numPhiSlices, helper);
  ...
  for (const auto& sp : spacepoints) {
    ...
    if (sp.r() < m_cfg.rMinMiddle)
      near_spacepoints.at(phislice).push_back(&sp);
    else if (sp.r() < m_cfg.rMaxMiddle)
      middle_spacepoints.at(phislice).push_back(&sp);
    else
      far_spacepoints.at(phislice).push_back(&sp);
  }

  std::vector<std::vector<std::vector<const spacepoint_t*>>>
      sorted_spacepoints = {near_spacepoints, middle_spacepoints,
                            far_spacepoints};
...
Acts::SeedVertexFinder<spacepoint_t>::findTriplets(
  ...
  for (int nearphi = 0; nearphi < m_cfg.numPhiSlices; ++nearphi) {
    for (const auto& near_sp : sorted_spacepoints.at(0).at(nearphi)) {
      ...
      for (int middlephi_h = nearphi - 1; middlephi_h <= nearphi + 1;
           ++middlephi_h) {
        ...
        for (const auto& middle_sp : sorted_spacepoints.at(1).at(middlephi)) {
          ...
          for (int farphi_h = middlephi - 1; farphi_h <= middlephi + 1;
               ++farphi_h) {
...
bool Acts::SeedVertexFinder<spacepoint_t>::isTripletValid(
  ...
  Acts::ActsScalar alpha1 = std::atan2(triplet.a->y() - triplet.b->y(),
                                       triplet.a->x() - triplet.b->x());
  // slope for middle+far spacepoints
  Acts::ActsScalar alpha2 = std::atan2(triplet.b->y() - triplet.c->y(),
                                       triplet.b->x() - triplet.c->x());
  ...
  // near-middle ray
  Acts::Vector3 ab{triplet.a->x() - triplet.b->x(),
                   triplet.a->y() - triplet.b->y(),
                   triplet.a->z() - triplet.b->z()};
  // middle-far ray
  Acts::Vector3 bc{triplet.b->x() - triplet.c->x(),
                   triplet.b->y() - triplet.c->y(),
